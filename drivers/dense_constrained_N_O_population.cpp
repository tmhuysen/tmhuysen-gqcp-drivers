...
int main(int argc, char** argv) {
    ...
    double min_range_population = 3.6;
    double max_range_population = 10.4;
    double interval_population = 0.01;
    ...
    try {
        ...
        desc.add_options()
                ...
                ("population,y", po::value<std::string>(&pop_string)->default_value(""), "population_sorts")
    ...
    for (size_t i = 0; i < target; i++) {
        std::string output_filename = atom_str1 + "_" + atom_str2 + "_" + distance_string + "_constrained_fci_dense_" + basisset + name + "_pop.out" + std::to_string(i);
